...
class ELMOCRFSegModel(LSTMCRFSegModel):
    ...
    def _setup_placeholders(self):
        self.placeholders = {'input_words': tf.placeholder(tf.int32,
                             ...
                             'input_length': tf.placeholder(tf.int32,
                                                            shape=[None]),
                             'elmo_vectors': tf.placeholder(
                                 tf.float32, shape=[None, 3, None, 1024]),
                             'seg_labels': tf.placeholder(tf.float32,
                                                          shape=[None, None]),
    ...
    def _embed(self):
        ...
        self.elmo_weights = tf.nn.softmax(
            tf.get_variable('elmo_weights', [3], dtype=tf.float32,
                            trainable=True))
        self.scale_para = tf.get_variable('scale_para', [1], dtype=tf.float32,
                                          trainable=True)
