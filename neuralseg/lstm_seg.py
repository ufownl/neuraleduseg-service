...
class LSTMSegModel(BaseSegModel):
    ...
    def _embed(self):
        with tf.device('/cpu:0'):
            ...
            self.word_embeddings = tf.get_variable(
                ...
                initializer=word_emb_init, trainable=False
            )
        ...
        self.embedded_inputs = tf.nn.dropout(
            self.embedded_words, self.placeholders['dropout_keep_prob']
        )

    def _encode(self):
        self.encoded_sent, _ = rnn('bi-lstm', self.embedded_inputs,
                                   self.placeholders['input_length'],
                                   hidden_size=self.hidden_size, layer_num=1,
                                   concat=True)
    ...
    def _output(self):
        self.u = tc.layers.fully_connected(self.encoded_sent, self.hidden_size,
                                           tf.nn.relu, scope='output_fc1')
        self.probs = tf.squeeze(tc.layers.fully_connected(
            self.u, 1, tf.nn.sigmoid, scope='output_fc2'), -1
        )

    def _compute_loss(self):
        self.losses = - tf.log(self.probs + 1e-9) \
                      * self.placeholders['seg_labels'] \
                      * self.sequence_mask \
                      - tf.log(1 - self.probs + 1e-9) \
                      * (1 - self.placeholders['seg_labels']) \
                      * self.sequence_mask
