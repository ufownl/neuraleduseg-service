...
def trilinear_similarity(x1, x2, scope='trilinear', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        weights_x1 = tf.get_variable('kernel_x1', [x1_shape[2], 1],
                                     dtype=x1.dtype)
        weights_x2 = tf.get_variable('kernel_x2', [x2_shape[2], 1],
                                     dtype=x2.dtype)
        weights_mul = tf.get_variable('kernel_mul', [1, 1, x1_shape[2]],
                                      dtype=x2.dtype)
        bias = tf.get_variable('bias', [1], dtype=x1.dtype,
...
def self_attention(inputs, lengths, window_size=-1, scope='bilinear_attention',
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if window_size > 0:
            restricted_mask = tf.matrix_band_part(
...
def rnn(rnn_type, inputs, length, hidden_size, layer_num=1,
    ...
    Args:
        ...
        dropout_keep_prob:
        concat: When the rnn is bidirectional, the forward outputs and
                backward outputs are concatenated if this is True, else
                we add them.
        scope: name scope
    ...
    with tf.variable_scope(scope, reuse=reuse):
        if not rnn_type.startswith('bi'):
            cell = get_cell(rnn_type, hidden_size, layer_num,
                            dropout_keep_prob)
            ...
            cell_fw = get_cell(rnn_type, hidden_size, layer_num,
                               dropout_keep_prob)
            cell_bw = get_cell(rnn_type, hidden_size, layer_num,
                               dropout_keep_prob)
            outputs, states = tf.nn.bidirectional_dynamic_rnn(
                cell_bw, cell_fw, inputs, sequence_length=length,
                dtype=tf.float32
            )
